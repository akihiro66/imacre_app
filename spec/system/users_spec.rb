...
RSpec.describe "Users", type: :system do
  ...
  describe "ユーザー登録ページ" do
    ...
    context "ページレイアウト" do
      it "「ユーザー登録」の文字列が存在する事を確認" do
        expect(page).to have_content 'ユーザー登録'
      end

      it "正しいタイトルが表示される事を確認" do
    ...
    context "ユーザー登録処理" do
      it "有効なユーザーでユーザー登録を行うとユーザー登録成功のフラッシュが表示されること" do
        ...
        click_button "登録する"
        expect(page).to have_content "イマクリへようこそ！"
      end
  ...
  describe "プロフィールページ" do
    ...
    context "お気に入り登録/解除" do
      ...
      it "作品のお気に入り登録/解除ができること" do
        expect(user.favorite?(production)).to be_falsey
        user.favorite(production)
        expect(user.favorite?(production)).to be_truthy
      ...
      it "お気に入り一覧ページが期待通り表示されること" do
        ...
        expect(page).not_to have_css ".favorite-production"
        user.favorite(production)
        user.favorite(other_production)
        visit favorites_path
        expect(page).to have_css ".favorite-production", count: 2
        expect(page).to have_content production.name
        expect(page).to have_content production.description
        expect(page).to have_content "created by #{user.name}"
        expect(page).to have_link user.name, href: user_path(user)
        expect(page).to have_content other_production.name
        expect(page).to have_content other_production.description
        expect(page).to have_content "created by #{other_user.name}"
        expect(page).to have_link other_user.name, href: user_path(other_user)
        user.unfavorite(other_production)
        visit favorites_path
        expect(page).to have_css ".favorite-production", count: 1
        expect(page).to have_content production.name
      end
    end
    ...
    context "通知生成" do
      ...
      context "自分以外のユーザーの作品に対して" do
        before do
          visit production_path(other_production)
        end
        ...
        it "お気に入り登録によって通知が作成されること" do
          ...
          expect(page).to have_css 'li.no_notification'
          expect(page).to have_content "あなたの作品が#{user.name}さんにお気に入り登録されました。"
          expect(page).to have_content other_production.name
          expect(page).to have_content other_production.description
          expect(page).to have_content other_production.created_at.strftime("%Y/%m/%d(%a) %H:%M")
        end
        ...
        it "コメントによって通知が作成されること" do
          ...
          visit notifications_path
          expect(page).to have_css 'li.no_notification'
          expect(page).to have_content "あなたの作品に#{user.name}さんがコメントしました。"
      ...
      context "自分の作品に対して" do
        ...
        it "お気に入り登録によって通知が作成されないこと" do
          ...
          expect(page).not_to have_content production.name
          expect(page).not_to have_content production.description
          expect(page).not_to have_content production.created_at
        end
        ...
        it "コメントによって通知が作成されないこと" do
          ...
          expect(page).not_to have_content other_production.name
          expect(page).not_to have_content other_production.description
          expect(page).not_to have_content other_production.created_at
        end
      end
    end
  end
