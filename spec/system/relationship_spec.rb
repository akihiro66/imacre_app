...
RSpec.describe "Relationships", type: :system do
  ...
  let!(:production2) { create(:production, user: user2) }
  let!(:production3) { create(:production, user: user3) }
  ...
  describe "フィード" do
    ...
    it "フィードに自分の投稿が含まれていること" do
      expect(user.feed).to include production
    end

    it "フィードにフォロー中ユーザーの投稿が含まれていること" do
      expect(user.feed).to include production2
    end

    it "フィードにフォローしていないユーザーの投稿が含まれていないこと" do
      expect(user.feed).not_to include production3
    end
  end
end
