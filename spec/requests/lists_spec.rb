...
RSpec.describe "リスト登録機能", type: :request do
  ...
  let(:production) { create(:production, user: other_user) }
  ...
  context "リスト登録/解除機能" do
    context "ログインしている場合" do
      ...
      it "作品のリスト登録/解除ができること" do
        expect {
          post "/lists/#{production.id}/create"
      ...
      it "作品のAjaxによるリスト登録/解除ができること" do
        expect {
          post "/lists/#{production.id}/create", xhr: true
        }.to change(other_user.lists, :count).by(1)
        expect {
    ...
    context "ログインしていない場合" do
      ...
      it "リスト解除は実行できず、ログインページへリダイレクトすること" do
        expect {
          delete "/lists/#{production.id}/destroy"
        }.not_to change(List, :count)
        expect(response).to redirect_to login_path
      end
    end
